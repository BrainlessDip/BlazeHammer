    ...
    async with semaphore:
        ...
        if result["success"]:
            ...
            if any([print_response, print_payload, print_headers]):
                ...
                if print_headers:
                    parser = custom_headers_parsers.get(status_code, custom_headers_parsers.get('all'))
                    output = parser(result["final_headers"]) if parser else "- Not found in `custom_headers_parsers`"
                    console.print(f"[[bold cyan]{timestamp}[/bold cyan] • [bold magenta]Headers[/bold magenta] • [bold green]{status_code}[/bold green]]\n{output}", justify="left")
                
                if print_payload:
                    parser = custom_payload_parsers.get(status_code, custom_payload_parsers.get('all'))
                    output = parser(result["final_payload"]) if parser else "- Not Found in 'custom_payload_parsers'"
                    console.print(f"[{timestamp} • [yellow]Payload[/yellow] • {status_code}]\n{output}", justify="left")
                
                if print_response:
                    parser = custom_response_parsers.get(status_code, custom_response_parsers.get('all'))
                    output = parser(result["response"]) if parser else "- Not found in `custom_response_parsers`"
                    console.print(f"[{timestamp} • [yellow]Response[/yellow] • {status_code}]\n{output}", justify="left")
...
async def run_load_test(
    ...
    for code, count in sorted(stats['status_codes'].items()):
        percent = (count / stats['success_count'] * 100) if stats['success_count'] else 0
...
def main():
    ...
    parser.add_argument("--json-diff", "-jd", nargs='+', metavar='FILE', help="Compare JSON files")
